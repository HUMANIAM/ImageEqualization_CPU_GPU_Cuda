...
PGM_IMG contrast_enhancement_g(PGM_IMG img_in)
{
    PGM_IMG result;
	int hist[256];
	int sz = img_in.w * img_in.h;
    
    result.w = img_in.w;
    result.h = img_in.h;
    result.img = (unsigned __int8  *)malloc(sz * sizeof(unsigned __int8 ));
    
    histogram(hist, img_in.img, sz, 256);
    histogram_equalization(result.img, img_in.img, hist, sz, 256);
    return result;
}
...
PPM_IMG contrast_enhancement_c_rgb(PPM_IMG img_in)
{
    PPM_IMG result;
	int hist[256];
	int sz = img_in.h * img_in.w;
    
    result.w = img_in.w;
    result.h = img_in.h;
    result.img_r = (unsigned __int8 *)malloc(sz * sizeof(unsigned __int8));
    result.img_g = (unsigned __int8 *)malloc(sz * sizeof(unsigned __int8));
    result.img_b = (unsigned __int8 *)malloc(sz * sizeof(unsigned __int8));
    
    histogram(hist, img_in.img_r, sz, 256);
    histogram_equalization(result.img_r, img_in.img_r, hist, sz, 256);

    histogram(hist, img_in.img_g, sz, 256);
    histogram_equalization(result.img_g, img_in.img_g, hist, sz, 256);

    histogram(hist, img_in.img_b, sz, 256);
    histogram_equalization(result.img_b, img_in.img_b, hist, sz, 256);
...
PPM_IMG contrast_enhancement_c_yuv(PPM_IMG img_in)
{
    ...
    unsigned __int8 * y_equ;
	int hist[256];
    
    yuv_med = rgb2yuv(img_in);
    y_equ = (unsigned __int8 *)malloc(yuv_med.h*yuv_med.w*sizeof(unsigned __int8));
...
PPM_IMG contrast_enhancement_c_hsl(PPM_IMG img_in)
{
    ...
    unsigned __int8 * l_equ;
	int hist[256];

    hsl_med = rgb2hsl(img_in);
	l_equ = (unsigned __int8 *)malloc(hsl_med.height*hsl_med.width*sizeof(unsigned __int8));
    ...
    free(hsl_med.h);    //free hue
    free(hsl_med.s);	//free saturation
    free(hsl_med.l);	//free lightness
    return result;
}
...
HSL_IMG rgb2hsl(PPM_IMG img_in)
{
	...
	img_out.l = (unsigned char *)malloc(img_in.w * img_in.h * sizeof(unsigned __int8));
    ...
    for(i = 0; i < img_in.w*img_in.h; i ++){
        ...
        img_out.h[i] = H;
        img_out.s[i] = S;
        img_out.l[i] = (unsigned __int8)(L*255);
    }
...
PPM_IMG hsl2rgb(HSL_IMG img_in)
{
    ...
    result.img_r = (unsigned __int8 *)malloc(result.w * result.h * sizeof(unsigned __int8));
    result.img_g = (unsigned __int8 *)malloc(result.w * result.h * sizeof(unsigned __int8));
    result.img_b = (unsigned __int8 *)malloc(result.w * result.h * sizeof(unsigned __int8));
...
YUV_IMG rgb2yuv(PPM_IMG img_in)
{
    ...
    unsigned __int8 y, cb, cr;
    
    img_out.w = img_in.w;
    img_out.h = img_in.h;
    img_out.img_y = (unsigned __int8 *)malloc(sizeof(unsigned __int8)*img_out.w*img_out.h);
    img_out.img_u = (unsigned __int8 *)malloc(sizeof(unsigned __int8)*img_out.w*img_out.h);
    img_out.img_v = (unsigned __int8 *)malloc(sizeof(unsigned __int8)*img_out.w*img_out.h);
    ...
    for(i = 0; i < img_out.w*img_out.h; i ++){
        ...
        y  = (unsigned __int8)( 0.299*r + 0.587*g +  0.114*b);
        cb = (unsigned __int8)(-0.169*r - 0.331*g +  0.499*b + 128);
        cr = (unsigned __int8)( 0.499*r - 0.418*g - 0.0813*b + 128);
